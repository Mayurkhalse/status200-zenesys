"""
Generator for CREDIT_NOTE documents.
"""
import os, random
...
def generate_single_credit_note(doc_id: str, config: dict, output_base_dir: str):
    template_num = random.randint(1, 10)
    template_id = f"cn_template_{template_num:02d}"
    industry = random.choice(config.get("industries", ["WHOLESALE"]))
    ...
    items = get_random_items(industry=industry, count=random.randint(1, 3))
    ...
    content = {
        ...
        "body_text": f"CREDIT NOTE / ADJUSTMENT VOUCHER.\nReason for Credit: Returned Goods / Price Difference\nOriginal Invoice Ref: INV-{random.randint(1000, 9999)}\nAmount Credited to Account: {currency} {total_amount:.2f}"
    }

    rel_dir = os.path.join("CREDIT_NOTE", f"template_{template_num:03d}")
    ...
    gt_data = {"document_id": doc_id, "credit_note_id": doc_id, "company_name": company, "counterparty_name": counterparty, "credited_amount": total_amount}
