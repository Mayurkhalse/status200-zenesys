"""
Generator for PURCHASE_ORDER documents.
"""
import os
import random
...
def generate_single_purchase_order(doc_id: str, config: dict, output_base_dir: str):
    template_num = random.randint(1, 10)
    template_id = f"po_template_{template_num:02d}"
    industry = random.choice(config.get("industries", ["MANUFACTURING"]))
    ...
    items = get_random_items(industry=industry, count=random.randint(2, 6))
    ...
    content = {
        ...
        "body_text": f"OFFICIAL PURCHASE ORDER.\nPlease supply the items listed below in accordance with PO conditions. Vendor Ref: {counterparty}."
    }

    rel_dir = os.path.join("PURCHASE_ORDER", f"template_{template_num:03d}")
    ...
    gt_data = {
        "document_id": doc_id,
        "po_number": doc_id,
        "po_date": content["date"],
        "buyer_company": company,
        "vendor_company": counterparty,
        "total_amount": total_amount,
        "currency": currency
    }
