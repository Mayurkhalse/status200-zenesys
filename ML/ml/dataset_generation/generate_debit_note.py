"""
Generator for DEBIT_NOTE documents.
"""
import os, random
...
def generate_single_debit_note(doc_id: str, config: dict, output_base_dir: str):
    ...
    content = {
        ...
        "body_text": f"DEBIT NOTE / SUPPLEMENTARY CHARGE VOUCHER.\nReason for Debit: Undercharged Freight / Quantity Revision\nOriginal Invoice Ref: INV-{random.randint(1000, 9999)}\nAdditional Debit Amount: {currency} {total_amount:.2f}"
    }

    rel_dir = os.path.join("DEBIT_NOTE", f"template_{template_num:03d}")
    ...
    gt_data = {"document_id": doc_id, "debit_note_id": doc_id, "company_name": company, "counterparty_name": counterparty, "debited_amount": total_amount}
