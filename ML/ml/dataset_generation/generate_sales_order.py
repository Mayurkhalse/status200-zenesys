"""
Generator for SALES_ORDER documents.
"""
import os, random
...
def generate_single_sales_order(doc_id: str, config: dict, output_base_dir: str):
    template_num = random.randint(1, 10)
    template_id = f"so_template_{template_num:02d}"
    industry = random.choice(config.get("industries", ["RETAIL"]))
    ...
    degradation = random.choice(config.get("degradation_types", ["rotation"])) if is_scanned else "none"
    ...
    content = {
        ...
        "body_text": f"CONFIRMATION OF SALES ORDER.\nWe acknowledge receipt of your order and hereby confirm the fulfillment details."
    }

    rel_dir = os.path.join("SALES_ORDER", f"template_{template_num:03d}")
    ...
    gt_data = {"document_id": doc_id, "sales_order_id": doc_id, "company_name": company, "counterparty_name": counterparty, "total_amount": total_amount}
